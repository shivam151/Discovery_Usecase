...
def get_s3_key_from_presigned_url(presigned_url: str) -> Optional[str]:
...
#class DiscoveryDatabase():
    ...
    def get_project_info(self, project_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            cursor.close()
            self.release_connection(conn)
    def delete_project_by_id(self, project_id: int) -> Optional[Dict[str, Any]]:
        """Delete a project, its associated S3 objects, and rely on CASCADE for related database records"""
        try:
            conn = self._get_connection()
            cursor = conn.cursor(cursor_factory=RealDictCursor)

        # Check if project exists
            cursor.execute("SELECT id, name, sow_path FROM projects WHERE id = %s", (project_id,))
        ...
        # Delete SOW file from S3 if it exists
            sow_path = project['sow_path']
            if sow_path:
                s3_key = get_s3_key_from_presigned_url(sow_path)
                if s3_key:
                    try:
                        s3_client.delete_object(Bucket=S3_BUCKET, Key=s3_key)
                        deleted_s3_objects.append(s3_key)
        ...
        # Fetch presigned URLs for additional documents
            cursor.execute(
                "SELECT file_path FROM additional_documents WHERE project_id = %s",
                (project_id,)
            )
            additional_docs = cursor.fetchall()
            for doc in additional_docs:
                presigned_url = doc['file_path']
                s3_key = get_s3_key_from_presigned_url(presigned_url)
                if s3_key:
                    try:
                        s3_client.delete_object(Bucket=S3_BUCKET, Key=s3_key)
                        deleted_s3_objects.append(s3_key)
        ...
        # Delete project (related tables will be deleted via CASCADE)
