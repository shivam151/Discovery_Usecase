...
class SOWParser:
    def __init__(self, gemini_api_key: str = None):
        ...
        if gemini_api_key:
            ...
            # Priority 3: Check environment variables
                ...
                resolved_gemini_api_key = os.environ["GOOGLE_API_KEY"]
        ...
        if not resolved_gemini_api_key:
            ...
            raise ValueError(
                "No Gemini API key provided. "
                "Please set GOOGLE_API_KEY in your .env file, environment variables, "
                "or pass it directly to the SOWParser constructor."
            )
    ...
    def parse_sow(self, file_input: Union[str, BytesIO], filename: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # Fallback to MIME type if extension is empty or invalid
            if not file_extension or file_extension not in ['.pdf', '.docx', '.pptx', '.txt', '.xlsx' , '.xls', '.csv' ]:
                logger.info(f"Empty or invalid file extension: {file_extension}, using MIME type fallback")
                if mime_type == 'application/pdf':
                    file_extension = '.pdf'
                    if not filename.lower().endswith('.pdf'):
                    ...
                    if not filename.lower().endswith('.txt'):
                        ...
                        logger.info(f"Appended .txt to filename: {filename}")  
                elif mime_type == 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet':
                    file_extension = '.xlsx'
                    if not filename.lower().endswith('.xlsx'):
                        filename = filename + '.xlsx'
                        logger.info(f"Appended .xlsx to filename: {filename}")
                elif mime_type == 'application/vnd.ms-excel':
                    file_extension = '.xls'
                    if not filename.lower().endswith('.xls'):
                        filename = filename + '.xls'
                        logger.info(f"Appended .xls to filename: {filename}")
                elif mime_type == 'text/csv':
                    file_extension = '.csv'
                    if not filename.lower().endswith('.csv'):
                        filename = filename + '.csv'
                        logger.info(f"Appended .csv to filename: {filename}")    
                else:
                    raise ValueError(f"Unsupported MIME type: {mime_type} for file: {filename}")
            ...
            if mime_type not in mime_map.get(file_extension, []):
                logger.warning(f"MIME type {mime_type} doesn't match expected types for {file_extension}, but proceeding anyway")
    ...
    def _extract_text_from_txt(self, file_content: BytesIO) -> str:
    ...
    def extract_text_from_excel(self, excel_bytes: bytes, filename: str) -> Dict[str, Any]:
        """
        Extract text content from Excel files (.xlsx, .xls)
        Returns {"text": extracted_text} if successful, {"error": error_message} if failed
        """
        try:
            # Ensure we have bytes, not BytesIO object
            ...
            for sheet_name in excel_file.sheet_names:
                try:
                    # Read the sheet with all data types as string to preserve formatting
                    df = pd.read_excel(excel_file, sheet_name=sheet_name, header=None, dtype=str)
                    
                    extracted_text += f"\n=== SHEET: {sheet_name} ===\n"
                    
                    for index, row in df.iterrows():
                        # Filter out NaN values and empty strings, then join with separator
                        row_values = [str(cell) for cell in row if pd.notna(cell) and str(cell).strip() != 'nan' and str(cell).strip()]
                    ...
                    extracted_text += f"\n=== ERROR READING SHEET '{sheet_name}': {str(sheet_error)} ===\n"
            
            # Check if any meaningful text was extracted
            clean_text = extracted_text.strip()
            content_lines = [line for line in clean_text.split('\n') if line.strip() and not line.startswith("===")]
            
            if not content_lines:
                return {
                    "status_code": 400,
                    "message": "Excel file does not contain readable data",
                    "data": None
                }
            ...
            return {"error": f"Failed to extract text from Excel file: {str(e)}"}

    def extract_text_from_csv(self, csv_bytes: bytes, filename: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not content_lines:
                return {
                    "status_code": 400,
                    "message": "CSV file does not contain readable data",
                    "data": None
                }
            ...
            return {"error": f"Failed to extract text from CSV file: {str(e)}"}
    ...
    def _extract_sections(self, document_text: str) -> Dict[str, str]:
        ...
        Document text:
        {document_text[:8000]}  # Truncate to avoid token limits
        """
    ...
    def _extract_requirements(self, sections: Dict[str, str]) -> List[Dict[str, str]]:
        ...
        try:
            ...
            logger.error(f"Gemini API call failed during requirement extraction: {str(e)}")
            return []
    ...
    def _identify_boundaries(self, sections: Dict[str, str]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Gemini API call failed during boundary identification: {str(e)}")
            return boundaries
